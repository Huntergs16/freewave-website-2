...
const Cart: React.FC<CartProps> = ({ cartItems }) => {
  ...
  const [cookies, setCookie, removeCookie] = useCookies<string, any>([
    "itemKeys",
  ]);

  useEffect(() => {
    if (!cartItems || !cartItems.length) {
      cartItems = cookies.itemKeys || [];
    }
          ...
          <div className='w-full flex flex-col gap-2'>
            ...
            <ul className='flex flex-col items-center w-full gap-4'>
              {cartItems.map((item) => (
                <li
                  key={item.id}
                  className='grid grid-cols-5 place-items-center w-full'
                >
            ...
            <div className='ml-auto gap-6 p-2 w-max max-w-[50vw] flex flex-col place-items-start'>
              ...
              <button className='border border-black w-full py-2 px-4 hover:opacity-70'>
                Check out
              </button>
            </div>
          </div>
